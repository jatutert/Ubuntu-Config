...
sudo echo 'Configuratie van' $NAME $VERSION 'gestart'
# Einde melding aanmaken
#
# Repository Ubuntu aanpassen naar nl.archive.ubuntu.com 
sudo sed 's@archive.ubuntu.com@nl.archive.ubuntu.com@' -i /etc/apt/sources.list
...
# Installatie tools 
sudo apt install -y mc screenfetch open-vm-tools wget
...
sudo apt purge -qq -y lxc-docker* || true 
# Instlalatie Docker-CE (DOCKER)
...
sudo chmod +x /home/$USER/pull-images.sh
