...
function ulx_os_config_nic () {
    ...
    if [ $hostname == "u24-lts-s-iacam-001" ] ; then
        curl -s -o /etc/netplan/01-netcfg.yaml https://raw.githubusercontent.com/jatutert/demos/main/Ansible/Guest/Ubuntu/Netplan/eth/fixed/sec-nic-eth-01-netcfg-dhcp-dns-iacam.yaml
        netplan apply 
    fi
    #
    # Ansible demo 
    ...
    if [ $hostname == "u24-lts-s-iacas-001" ] ; then
        curl -s -o /etc/netplan/01-netcfg.yaml https://raw.githubusercontent.com/jatutert/demos/main/Ansible/Guest/Ubuntu/Netplan/eth/fixed/sec-nic-eth-01-netcfg-dhcp-dns-iacas.yaml
        netplan apply 
    fi
    # 
    # Docker Demo
    ...
    if [ $hostname == "u24-lts-s-dckr-001" ] ; then
        curl -s -o /etc/netplan/01-netcfg.yaml https://raw.githubusercontent.com/jatutert/demos/main/Docker/Guest/Ubuntu/Netplan/eth/fixed/sec-nic-eth-01-netcfg-dhcp-dns-dckr.yaml
        netplan apply 
    fi
    #
