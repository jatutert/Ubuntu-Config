...
#
if [ $# -eq 0 ]; then
    echo "Geen parameter opgegeven. Gebruik 'upgrade' 'docker' 'minikube' 'ansible'."
...
function ulx_os_change_repo_nl () {
    ...
    if [ $VERSION_ID == "22.04" ] ; then
        if grep -q "mirrors.edge.kernel.org" /etc/apt/sources.list; then
            ...
            echo "Mirrors.edge.kernel.org is geen actieve repository en dus geen actie ondernomen"
        fi
        #
...
function ulx_os_netplan_download () {
    ...
    if [ $hostname == "u24-lts-s-iacam-001" ] ; then
        curl -s -o /etc/netplan/iacam-netcfg.yaml https://raw.githubusercontent.com/jatutert/demos/main/Ansible/Guest/Ubuntu/Netplan/eth/fixed/sec-nic-eth-01-netcfg-dhcp-dns-iacam.yaml
    fi
    #
    # Ansible demo 
    #
    ...
    if [ $hostname == "u24-lts-s-iacas-001" ] ; then
        curl -s -o /etc/netplan/iacas-netcfg.yaml https://raw.githubusercontent.com/jatutert/demos/main/Ansible/Guest/Ubuntu/Netplan/eth/fixed/sec-nic-eth-01-netcfg-dhcp-dns-iacas.yaml
    fi
    # 
    # Docker Demo
    #
    ...
    if [ $hostname == "u24-lts-s-dckr-001" ] ; then
        curl -s -o /etc/netplan/dckr-netcfg.yaml https://raw.githubusercontent.com/jatutert/demos/main/Docker/Guest/Ubuntu/Netplan/eth/fixed/sec-nic-eth-01-netcfg-dhcp-dns-dckr.yaml
    fi
    #
    ...
    if [ $hostname == "u24-lts-s-dbms-001" ] ; then
        curl -o /etc/netplan/dbms-netcfg.yaml https://raw.githubusercontent.com/jatutert/demos/main/OSTicket/Guest/Ubuntu/Netplan/Databaseserver/eth/eth-sec-nic-01-netcfg-dbms.yaml
    fi
    #
    # Introductie Infrastructuren
    #
    ...
    if [ $hostname == "u24-lts-s-wsrv-001" ] ; then
        curl -o /etc/netplan/wsrv-netcfg.yaml https://raw.githubusercontent.com/jatutert/demos/main/OSTicket/Guest/Ubuntu/Netplan/Webserver/eth/eth-sec-nic-01-netcfg-wsrv.yaml
    fi
}
#
