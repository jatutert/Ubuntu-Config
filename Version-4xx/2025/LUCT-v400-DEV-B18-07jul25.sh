...
function ulx_install_docker () {
    ...
    if ! [ -x "$(command -v podman)" ]; then
        ...
        if ! [ -x "$(command -v docker)" ]; then
            ...
            #
            curl -sSL https://get.docker.com/ | sh > /home/$SUDO_USER/luct_logs/luct_docker.log 2>&1
            #
...
function ulx_nested_oobe () {
    ...
    ulx_os_upgrade_os
    echo 'DEBIAN/UBUNTU - Step 3 of 8 Changing Networking settings (Skipped in this Build)'
