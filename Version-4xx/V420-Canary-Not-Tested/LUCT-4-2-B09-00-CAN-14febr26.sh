...
#
#
function debulx_conteng_cockpit_run () {
    #
    ...
    local containerengine=$1
    #
    $containerengine run -d --name LUCT_Cockpit --privileged -v /:/host -p 9109:9090 --restart always quay.io/cockpit/w
    #
# Cockpit
}
#
...
function debulx_nested_conteng_complete () {
    ...
    # ##########################################################################
    #
    echo "## Phase 10 - Step 3A of 10 Starting Jenkins CI CD CD Server as container on $actie"
    ...
    # ##########################################################################
    #
    echo '## Phase 10 - Step 3B of 10 Starting Portainer as container'
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
    ...
    # ##########################################################################
    #
    echo '## Phase 10 - Step 3C of 10 Starting Yacht as container'
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
    ...
    # ##########################################################################
    #
    echo '## Phase 10 - Step 3D of 10 Starting Visual Studio Code Server as container'
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
    ...
    # ##########################################################################
    #
    echo '## Phase 10 - Step 3E of 10 Starting Registry as container'
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
    ...
    # ##########################################################################
    #
    echo '## Phase 10 - Step 3F of 10 Starting WatchTower as container'
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
    ...
    # ##########################################################################
    #
    echo '## Phase 10 - Step 3G of 10 Starting Cockpit as container'
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
        debulx_conteng_cockpit_run "docker"
    fi
    if [[ $actie == "podman" ]]; then
        debulx_conteng_cockpit_run "podman"
    fi
    #
    # ##########################################################################
    #
    echo '## Phase 10 - Step 3H of 10 Starting PostGreSQL as container'
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
    ...
    # ##########################################################################
    #
    echo '## Phase 10 - Step 3I of 10 Starting PGAdmin4 as container'
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
    ...
    # ##########################################################################
    #
    echo "## Phase 10 - Step 4 of 10 Installing Container Engine Management tools"
    if [[ $actie == "docker" ]]; then
    ...
    # ##########################################################################
    #
    echo "## Phase 10 - Step 5 of 10 Saving Jenkins initialAdminPassword to file"
    if [[ $actie == "docker" ]]; then
