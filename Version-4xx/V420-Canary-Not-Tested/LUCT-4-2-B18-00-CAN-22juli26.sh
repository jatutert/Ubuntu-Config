...
readonly LUCT_Minor_Version_Build="18"
...
function debulx_os_upgrade_packages () {
    ...
    if [[ $modus == "test" ]]; then
        ...
        apt install python3-distupgrade -y > /dev/null 2>&1 || true
    fi
    if [[ $modus = "test" ]]; then
...
#
#
    # Start de installatie op de achtergrond (&)
    sudo apt install "${APT_INSTALL_ARRAY[@]}" -y > /dev/null 2>&1 &
    
    # Pak het proces-ID van het laatste commando
    local apt_pid=$!
    
    # Start de spinner en geef het PID mee
