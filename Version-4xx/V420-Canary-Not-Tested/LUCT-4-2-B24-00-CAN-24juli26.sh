...
#   24juli26    24  WSL versie 2 compabiliteit verhogen 
#
#   #######################
#
#
...
function luct_display_header () {
    echo '#########################################################################################'
    echo "Linux Universal Configuration Tool (LUCT) Version $LUCT_Major_Version.$LUCT_Minor_Version Build $LUCT_Minor_Version_Build Patch $LUCT_Minor_Version_Build_Patch"
    if [[ $LUCT_ReleaseCandidate == "0" ]]; then
        echo "Channel $LUCT_Channel"
    else 
        echo "Channel $LUCT_Channel                      RELEASE CANDIDATE $LUCT_ReleaseCandidate"
    fi
...
#
#
logger "LUCT $LUCT_Major_Version.$LUCT_Minor_Version Build $LUCT_Minor_Version_Build $LUCT_Minor_Version_Build_Patch Channel $LUCT_Channel Started by $USER"
logger "Timezone changed by LUCT $LUCT_Major_Version.$LUCT_Minor_Version Build $LUCT_Minor_Version_Build $LUCT_Minor_Version_Build_Patch Channel $LUCT_Channel"
#
#
luct_log_message "#######################################################"
luct_log_message "LUCT $LUCT_Major_Version.$LUCT_Minor_Version Build $LUCT_Minor_Version_Build $LUCT_Minor_Version_Build_Patch Channel $LUCT_Channel"
luct_log_message "#######################################################"
...
luct_log_message "Phase $phase $(date)"
#
if [ $(id -u) -ne 0 ]; then
    clear
    luct_display_header
    echo "## Phase $phase - Checking executing rights"
    echo ''
    ...
    logger "LUCT Version $LUCT_Major_Version.$LUCT_Minor_Version Build $LUCT_Minor_Version_Build Patch $LUCT_Minor_Version_Build_Patch Terminated at Phase 1"
...
luct_log_message "Phase $phase $(date)"
#
echo "## Phase $phase - Checking Script Arguments"
if [ $# -eq 0 ]; then
    ...
    luct_log_message "Afbreken script $(date)"
    #
    logger "LUCT Version $LUCT_Major_Version.$LUCT_Minor_Version Build $LUCT_Minor_Version_Build Patch $LUCT_Minor_Version_Build_Patch Terminated at Phase 2"
    exit 1
fi
#
#
...
#
#
luct_log_message "Phase $phase $(date) Checking running mode script $USER ${SUDO_USER}"
#
echo "## Phase #phase - Checking Script running mode"
#
#
# SET E
# De betere aanpak is set -euo pipefail wel inschakelen en kritieke commando's voorzien van || true waar een fout acceptabel is, en expliciete foutmeldingen waar dat niet het geval is 
set -euo pipefail
#
#
# SET X
...
# #######################
# Blok 3A
# Vullen variabelen op basis van aangetroffen Linux Distro      PHASE 4
# #######################
#
#
(( phase = phase + 1 ))
#
#
luct_log_message "Phase $phase $(date) Current Linux distro info $USER ${SUDO_USER}"
#
echo "## Phase $phase - Collecting current Linux Distribution information"
#
...
#
#
luct_log_message "Phase $phase $(date) Installing or updating tools needed for script $USER ${SUDO_USER}"
#
#
# #######################
# Blok 4A
# Installing or updating tools needed for this script       PHASE 5
#
#   LET OP
#   Maakt SED beschikbaar voor Stap 4B
#
# #######################
#
#
echo "## Phase $phase - Step 1 of 2 Installing or updating tools needed for this script"
#
. /etc/os-release
distro=$(echo "$ID" | tr '[:upper:]' '[:lower:]')
versie=$(echo "$VERSION_ID" | tr '[:upper:]' '[:lower:]')
...
#
#
echo "## Phase $phase - Step 2 of 2 Changing Operating System Repository"
#
. /etc/os-release
distro=$(echo "$ID" | tr '[:upper:]' '[:lower:]')
versie=$(echo "$VERSION_ID" | tr '[:upper:]' '[:lower:]')
...
if [[ $distro == "debian" ]] ; then
    if [[ $versie == "12" ]] ; then
        sed -i '1c\deb https://mirror.nl.mirhosting.net/debian/ bookworm main non-free non-free-firmware' /etc/apt/sources.list
...
#
#
luct_log_message "Phase $phase $(date) Define functions $USER ${SUDO_USER}"
#
echo "## Phase $phase - Define Functions for this Script"
#
#
# ################################################################################
#
...
function build_install_docker_compose_plugin () {
    ...
    #
    # Versienummer ophalen latest
    #
    DCMP_VERSION=$(curl -s https://api.github.com/repos/docker/compose/releases/latest | jq -r '.tag_name' | tr -d 'v')
    #
    curl -s -SL "https://github.com/docker/compose/releases/download/v$DCMP_VERSION/docker-compose-linux-x86_64" -o /home/${SUDO_USER}/.docker/cli-plugins/docker-compose
    #
...
function debulx_os_machine_init () {
    echo "[INFO] Stoppen van systemd-machine-id-setup (indien actief)..."

    # Verwijder bestaande machine-id bestanden
    echo "[INFO] Verwijderen van huidige machine-id..."
    rm -f /etc/machine-id
    rm -f /var/lib/dbus/machine-id

    # Leeg bestand opnieuw aanmaken (vereist door systemd bij boot)
    echo "[INFO] Nieuwe lege machine-id initialiseren..."
    touch /etc/machine-id

    # Nieuwe machine-id genereren
    echo "[INFO] Genereren van nieuwe machine-id..."
    systemd-machine-id-setup

    # Synchroniseer dbus machine-id (compatibiliteit)
    echo "[INFO] Synchroniseren met D-Bus machine-id..."
    ln -sf /etc/machine-id /var/lib/dbus/machine-id

    # Toon resultaat
    echo "[INFO] Nieuwe machine-id:"
    cat /etc/machine-id

    echo "[INFO] Machine-id regeneratie voltooid."
...
function debulx_os_update_apt () {
    apt update -qq || true > /dev/null 2>&1
}
#
#
#
...
function debulx_os_upgrade_packages () {
    ...
    if [[ $modus == "test" ]]; then
        ...
        apt full-upgrade -qq -y || true > /dev/null 2>&1
        apt install python3-distupgrade -qq -y || true > /dev/null 2>&1
    fi
    if [[ $modus = "test" ]]; then
...
function debulx_install_default_apps () {
    ...
    if [[ $distro == "debian" ]]; then
        ...
        apt install -t ${VERSION_CODENAME}-backports snapd -qq -y || true >> /home/${SUDO_USER}/luct-logs/debulx_install_default_apps_snap.log 2>&1
        snap install core || true >> /home/${SUDO_USER}/luct-logs/debulx_install_default_apps_snap.log 2>&1
        #
    fi
    if [[ $distro == "ubuntu" ]]; then
        #
    ...
    # Ophalen laatste versienummer
    BAT_VERSION=$(curl -s https://api.github.com/repos/sharkdp/bat/releases/latest | jq -r '.tag_name' | tr -d 'v')
    #
    wget -q -O /tmp/bat_amd64.deb "https://github.com/sharkdp/bat/releases/download/v${BAT_VERSION}/bat_${BAT_VERSION}_amd64.deb"
    #   wget -q -O /tmp/bat_0.25.0_amd64.deb https://github.com/sharkdp/bat/releases/download/v0.25.0/bat_0.25.0_amd64.deb
    dpkg -i /tmp/bat_amd64.deb || true > /dev/null 2>&1
    #   dpkg -i /tmp/bat_${BAT_VERSION}_amd64.deb > /dev/null 2>&1
...
function debulx_install_cockpit_srv () {
    ...
    if ! systemctl is-active --quiet cockpit.socket; then
        ...
        # ######################
        #
        apt install -t ${VERSION_CODENAME}-backports cockpit -qq -y > /home/${SUDO_USER}/luct-logs/debulx_install_cockpit_srv.log 2>&1
        #
        # ######################
        # ## Service
        # ######################
...
#
#
function debulx_purge_container_engines () {
    for pkg in container docker podman runc; do
        apt purge "${pkg}*" -y > /dev/null 2>&1 || true
    done
...
function debulx_install_conteng () {
    ...
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
        # 
        debulx_purge_container_engines
        #
        ...
        for apt_docker_install in "${APT_DOCKER_INSTALL_ARRAY[@]}"; do
            ...
            apt install "$apt_docker_install" -qq -y >> /home/${SUDO_USER}/luct-logs/debulx_install_docker.log 2>&1
            if [[ $? -ne 0 ]]; then
    ...
    if [[ $actie == "podman" ]]; then
        ...
        apt install podman -qq -y > /home/${SUDO_USER}/luct-logs/debulx_install_podman.log 2>&1
        ...
        apt install cockpit-podman -qq -y >> /home/${SUDO_USER}/luct-logs/debulx_install_podman.log 2>&1
...
function debulx_install_pwrshell () {
    ...
    #   if [ $? -ne 0 ]; then
    if ! pwsh --command Get-Host; then 
        luct_log_message "Installing Powershell 7"
        #
        ...
        #
        apt install powershell -qq -y || true >> /home/${SUDO_USER}/luct-logs/luct_pwsh.log 2>&1
...
function debulx_install_minikube_and_k8stools () {
    ...
    #
    apt install kubeadm kubectl kubecolor -qq -y > /dev/null 2>&1
    #
...
#
# ################################################
# Blok 5E-3-11   Container Engine
#                Naam
# ###############################################
#
#
function debulx_conteng_naam_run () {
    #
    luct_log_message "naam" 
    #
    if [[ $modus == "test" ]]; then
        echo 'naam'
    fi
...
function debulx_config_os_repo_change () {
    #
    #
    #   Alleen nog voor LinuxMint
    #   Overige distros worden al in fase 5 aangepast
    #
    #
    #
    # Debian
    ...
    if [[ $distro == "debian" ]] ; then
        if [[ $versie == "12" ]] ; then
            echo 'In fase 5 is dit al gedaan. Dus ik heb niets te doen ...'
            #   sed -i '1c\deb https://mirror.nl.mirhosting.net/debian/ bookworm main non-free non-free-firmware' /etc/apt/sources.list
            #   sed -i '2c\deb-src https://mirror.nl.mirhosting.net/debian/ bookworm main non-free non-free-firmware' /etc/apt/sources.list
        fi
        if [[ $versie == "13" ]] ; then
    ...
    if [[ $distro == "linuxmint" ]] ; then
        ...
        #
        #   LinuxMint Nieuw bestand aanmaken met aangepaste inhoud
        #
        #   # LinuxMint Provider Triple IT
...
function debulx_config_add_repositories () {
    ...
    if [[ $distro = "debian" || "ubuntu" ]]; then
        ...
        #
        #   Officiele manier
        #   echo 'deb mirror://mirrors.ubuntu.com/mirrors.txt noble main restricted universe multiverse' | sudo tee -a /etc/apt/sources.list > /dev/null
        #
    fi
    if [[ $distro = "linuxmint" ]]; then
        add-apt-repository http://mirror.pilotfiber.com/$distro/ -s -y > /dev/null 2>&1
    ...
    # ############################
    #
    #   luct_log_message "Debian Backports" 
    #
    #   if [[ $distro == "debian" ]]; then
    #       echo "deb http://deb.debian.org/debian ${VERSION_CODENAME}-backports main" > /etc/apt/sources.list.d/backports.list
    #   fi
    #
    #   Slaan we over omdat in begin van het script al gedaan is
    #   Verwijderen in DEV versie van dit script 
    #
    #
    #
    # ############################
...
function debulx_config_virtualization () {
    ...
    if sudo dmidecode | grep -iq vmware; then
        #
        apt install open-vm-tools -qq -y > /dev/null 2>&1
...
function debulx_nested_conteng_complete () {
    ...
    # ##########################################################################
    #
    echo "## Phase 10 - Step 1 of 5 Installation and configuration of Container Engine"
    ...
    # ##########################################################################
    #
    echo '## Phase 10 - Step 2 of 5 Pull most commonly used Images from Docker Hub'
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
    ...
    #
    echo '## Phase 10 - Step 3 of 5 Spinning up images as containers'
    #
    # ##########################################################################
    #
    echo "## Phase 10 - Step 3J1 - Starting Jenkins CI CD CD Server as container on $actie"
    ...
    if [[ $modus == "databases" ]]; then
        #
        echo '## Phase 10 - Step 3P1 - Starting PGAdmin4 as container'
        if [[ $actie == "docker" || $actie == "minikube" ]]; then
            debulx_conteng_pgadmin4_run "docker"
        fi
        if [[ $actie == "podman" ]]; then
            debulx_conteng_pgadmin4_run "podman"
        fi
    fi
    #
    # ##########################################################################
    #
    echo '## Phase 10 - Step 3P2 - Starting Portainer as container'
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
    ...
    if [[ $modus == "databases" ]]; then
        #
        echo '## Phase 10 - Step 3P3 - Starting PostGreSQL as container'
        if [[ $actie == "docker" || $actie == "minikube" ]]; then
            debulx_conteng_postgresql_run "docker"
        fi
        if [[ $actie == "podman" ]]; then
            debulx_conteng_postgresql_run "podman"
        fi
    fi
    #
    # ##########################################################################
    #
    echo '## Phase 10 - Step 3R1 - Starting Registry as container'
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
    ...
    # ##########################################################################
    #
    echo '## Phase 10 - Step 3V1 - Starting Visual Studio Code Server as container'
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
    ...
    # ##########################################################################
    #
    echo '## Phase 10 - Step 3W1 - Starting WatchTower as container'
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
    ...
    # ##########################################################################
    #
    echo '## Phase 10 - Step 3Y1 - Starting Yacht as container'
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
    ...
    # ##########################################################################
    #
    echo "## Phase 10 - Step 4 of 5 Installing Container Engine Management tools"
    if [[ $actie == "docker" ]]; then
    ...
    # ##########################################################################
    #
    echo "## Phase 10 - Step 5 of 5 Saving Jenkins initialAdminPassword to file"
    if [[ $actie == "docker" ]]; then
...
function luct_linux_oobe () {
    ...
    (( phase = phase + 1 ))
    echo "## Phase $phase - Configure Operating System"
    if [[ $distro == "debian" || $distro == "ubuntu" ]]; then
    ...
    # GitHub Clones maken
    (( phase = phase + 1 ))
    echo "## Phase $phase - Cloning GitHub Repositories"
    git_clone_demos
    #
...
function verander_machinenaam () {
    #
    luct_log_message "Phase $phase $(date) $USER ${SUDO_USER}"
    #
    #   Alleen uitvoeren indien NIET gelijk aan WSL
    #
    kernel_name=$(uname -r)
    if [[ "${SUDO_USER}" != "vagrant" || "${kernel_name,,}" != *"wsl"* || "$kernel_name" != *-WSL2 ]]; then
        if [[ $distro == "debian" ]] ; then
            . /etc/os-release
...
function maak_directories () {
    ...
    #
    #
    echo "## Phase $phase - Step 1 Creating tmp directory in home directory of this user"
    ...
    #
    #
    echo "## Phase $phase - Step 2 Creating lab directory in home directory of this user"
    ...
    #
    #
    echo "## Phase $phase - Step 2 Creating LUCT logging directory"
    ...
    #
    if [[ $actie == "docker" || $actie == "podman" || $actie == "minikube" ]]; then
        echo "## Phase $phase - Step 3 Creating Container Engine scripts directory"
        if [[ $actie == "docker" || $actie == "minikube" ]]; then
...
function git_clone_demos () {
    #
    luct_log_message "Phase $phase Starttijd $(date) $USER ${SUDO_USER}"
    #
    # Demos
    echo "## Phase $phase - Clone GitHub JATUTERT Demos"
    if [ -d "/home/${SUDO_USER}/demos" ]; then
        rm -rf "/home/${SUDO_USER}/demos"
    fi
    git clone --quiet https://github.com/jatutert/demos.git /home/${SUDO_USER}/demos
    #
    # Awesome Compose
    if [[ $actie == "docker" ]] ; then
        echo "## Phase $phase - Clone GitHub Awesome Compose"
        if [ -d "/home/${SUDO_USER}/demos" ]; then
            rm -rf "/home/${SUDO_USER}/demos/Docker/Compose/Awesome-compose"
        fi
    ...
    #
    # Onderwijs
    echo "## Phase $phase - Clone GitHub Onderwijs"
    if [ -d "/home/${SUDO_USER}/onderwijs" ]; then
        rm -rf "/home/${SUDO_USER}/onderwijs"
    fi
    ...
    #
    # PowerShell
    echo "## Phase $phase - Clone GitHub Powershell is fun"
    ...
    #
    # Hedy
    echo "## Phase $phase - Clone GitHub Hedy"
    if [ -d "/home/${SUDO_USER}/hedy" ]; then
        rm -rf "/home/${SUDO_USER}/hedy"
    fi
    git clone --quiet --depth 1 https://github.com/hedyorg/hedy.git /home/${SUDO_USER}/hedy
    #
    # Ansible 
    if [[ $actie == "iacmaster" ]] ; then
        echo "## Phase $phase - Clone GitHub Ansible for DevOps"
    ...
    #
    echo "## Phase $phase - Make ${SUDO_USER} user owner of all directories"
    ...
    chown -f -R ${SUDO_USER}:${SUDO_USER} /home/${SUDO_USER}/hedy
    if [ -d "/home/${SUDO_USER}/ansible_devops" ]; then
        chown -f -R ${SUDO_USER}:${SUDO_USER} /home/${SUDO_USER}/ansible_devops
    fi
    #
    # Shell Scripts
    echo "## Phase $phase - Make all Shell Scriptfiles Executable"
...
function luct_finish_script () {
    ...
    #
    #
    luct_log_message "Fase $phase Eindtijd van het script $(date)"
    #
    clear
    luct_display_header
    echo ''
