...
readonly LUCT_Minor_Version_Build="21"
...
function debulx_install_default_apps () {
    ...
    for apt_install in "${APT_INSTALL_ARRAY[@]}"; do
        ...
        if [[ $distro == "debian" ]]; then
            apt install -t ${VERSION_CODENAME}-backports "$apt_install" -qq -y || true >> /home/$SUDO_USER/luct-logs/debulx_install_default_apps_pb.log 2>&1
        fi
        if [[ $distro == "ubuntu" ]]; then
