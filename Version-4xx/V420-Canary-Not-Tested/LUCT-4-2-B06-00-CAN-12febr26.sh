...
echo '- Ubuntu  24 LTS  Noble Numbat on WSL2'
echo ''
    ...
    if [[ $distro == "ubuntu" ]] ; then
        ...
        if [[ $versie == "22.04" ]] ; then
            if grep -q "in.archive.ubuntu.com" /etc/apt/sources.list; then
                #   werkt niet meer sed "s@in.archive.ubuntu.com@mirror.ams.macarne.com@" -i /etc/apt/sources.list
            fi
            if grep -q "nl.archive.ubuntu.com" /etc/apt/sources.list; then
                #   werkt niet meer sed "s@nl.archive.ubuntu.com@mirror.ams.macarne.com@" -i /etc/apt/sources.list
            fi
            if grep -q "mirrors.edge.kernel.org" /etc/apt/sources.list; then
                #   werkt niet meer sed "s@mirrors.edge.kernel.org@mirror.ams.macarne.com@" -i /etc/apt/sources.list
            fi 
        fi
        #
        if [[ $versie == "24.04" ]] ; then
            ...
            jq moet eerst geinstalleerd worden 
            ...
            # Vervangen mirror
            #
            sudo sed "s@archive.ubuntu.com@$ubuntu_mirror_nieuw@" -i /etc/apt/sources.list.d/ubuntu.sources


            # WSL2
            kernel_name=$(uname -r)
            # Controleer of "WSL" (niet hoofdlettergevoelig) voorkomt in de string
            if [[ "$kernel_name" == *"WSL"* ]] || [[ "$kernel_name" == *"wsl"* ]]; then
                #   OS      :   Ubuntu 24.04 noble(on the Windows Subsystem for Linux)
                #   Kernel  :   x86_64 Linux 6.6.87.2-microsoft-standard-WSL2
                sudo sed "s@archive.ubuntu.com@mirror.i3d.net/pub/ubuntu@" -i /etc/apt/sources.list.d/ubuntu.sources
            fi
            ...
            sed -i '2c\URIs: http://mirror.i3d.net/pub/ubuntu' /etc/apt/sources.list.d/ubuntu.sources > /home/$SUDO_USER/luct-logs/add-apt-repository.log
            #   Werkt niet meer     sed -i '2c\URIs: http://mirror.ams.macarne.com/ubuntu/' /etc/apt/sources.list.d/ubuntu.sources > /home/$SUDO_USER/luct-logs/add-apt-repository.log
        # 24.04 Einde
        fi
    # Ubuntu Einde 
    fi
