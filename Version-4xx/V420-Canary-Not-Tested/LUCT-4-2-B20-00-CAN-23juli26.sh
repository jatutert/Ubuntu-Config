...
readonly LUCT_Minor_Version_Build="20"
...
if [[ $distro == "debian" || $distro == "ubuntu" ]]; then
    if [[ $versie == "13" ]] ; then
        apt update -qq || true
        apt install curl jq sed wget wget2 -y -qq || true
    fi
...
function debulx_os_upgrade_packages () {
    ...
    if [[ $modus == "test" ]]; then
        ...
        apt install python3-distupgrade -y || true
    else
        apt full-upgrade -y || true > /dev/null 2>&1
        apt install python3-distupgrade -y || true > /dev/null 2>&1
    fi
    if [[ $modus = "test" ]]; then
        apt autoremove -y || true 
    else
        apt autoremove -y || true > /dev/null 2>&1
    fi
    #
...
function debulx_install_default_apps () {
    ...
    for apt_install in "${APT_INSTALL_ARRAY[@]}"; do
        ...
        if [[ $distro == "debian" ]]; then
            apt install -t ${VERSION_CODENAME}-backports "$apt_install" -y || true >> /home/$SUDO_USER/luct-logs/debulx_install_default_apps_pb.log 2>&1
        fi
        if [[ $distro == "ubuntu" ]]; then
            apt install "$apt_install" -y || true >> /home/$SUDO_USER/luct-logs/debulx_install_default_apps_pb.log 2>&1
        fi
        if [[ $? -ne 0 ]]; then
    ...
    if [[ $distro == "debian" ]]; then
        ...
        apt install -t ${VERSION_CODENAME}-backports snapd -y || true >> /home/${SUDO_USER}/luct-logs/debulx_install_default_apps_snap.log 2>&1
...
function debulx_install_pwrshell () {
    ...
    if ! pwsh --command Get-Host; then 
        ...
        #
        apt install powershell -y || true >> /home/${SUDO_USER}/luct-logs/luct_pwsh.log 2>&1
