...
for apt_install in "${APT_INSTALL_ARRAY[@]}"; do
    ...
    apt install "$apt_install" -y >> /home/$SUDO_USER/luct_logs/debulx_install_default_apps_pb.log 2>&1
    if [ $? -ne 0 ]; then
...
echo " "
    ...
    if [ $distro == "debian" ]; then
        ...
        #
        apt install snapd -y >> /home/$SUDO_USER/luct_logs/debulx_install_default_apps_snap.log 2>&1
        snap install core >> /home/$SUDO_USER/luct_logs/debulx_install_default_apps_snap.log 2>&1
        #
    fi
...
snap install curl >> /home/$SUDO_USER/luct_logs/debulx_install_default_apps_snap.log 2>&1
...
function debulx_install_podman () {
    ...
    if ! [ -x "$(command -v docker)" ]; then
        ...
        if ! [ -x "$(command -v podman)" ]; then
            ...
            service podman start >> /home/$SUDO_USER/luct_logs/debulx_install_podman.log 2>&1
...
function debulx_podman_images_pull () {
    ...
    if [ $modus == "test" ]; then
        ...
        podman images > /home/$SUDO_USER/luct_logs/debulx_docker_images_pull.log 2>&1
    fi
    #
}
#
...
function debulx_podman_portainer_run () {
    ...
    # Port 9443: Used by the Portainer Server for accessing the user interface (UI) and API. This is the default HTTPS port for Portainer
    # Port 9001: The default port used by the Portainer Agent to communicate with the Portainer Server. 
    podman run -d -p 8000:8000 -p 9101:9443 --name LUCT_portainer --restart=always -v /var/run/docker.sock:/var/run/docker.sock -v portainer_data:/data portainer/portainer-ce:latest >> /home/$SUDO_USER/luct_logs/luct_start_Portainer.log 2>&1
    #
    # Bash Shell Script maken Portainer
    echo '#! /bin/bash' > /home/$SUDO_USER/portainer_restart.sh
    #
...
function debulx_podman_yacht_run () {
    #
    # Docker Volume Aanmaken voor Yacht
    podman volume create yacht_data > /home/$SUDO_USER/luct_logs/debulx_docker_yacht_run.log 2>&1
    #
    # Docker Run Yacht
    podman run -d -p 9102:8000 -v /var/run/docker.sock:/var/run/docker.sock -v yacht_data:/config --name LUCT_Yacht --restart=always selfhostedpro/yacht >> /home/$SUDO_USER/luct_logs/luct_start_Yacht.log 2>&1
    #
    # Bash Shell Script maken Yacht
    echo '#! /bin/bash' > /home/$SUDO_USER/yacht_restart.sh
    #
...
function debulx_podman_vscsrv_run () {
   ...
   # Start Visual Studio Code Server als Container
   #
   podman run -d -p 9103:8080 -v "$PROJECT_DIR:/home/coder/project" --name "$CONTAINER_NAME" --restart=always "$DOCKER_IMAGE" --auth=none >> /home/$SUDO_USER/luct_logs/luct_start_vsc_srv.log 2>&1
   #
   # In Visual Studio Code Server is lokale omgeving zichtbaar onder Project
   #
}
...
function debulx_podman_jenkins_run () {
    #
    # Docker Volume Aanmaken voor Yacht
    podman volume create jenkins_data > /home/$SUDO_USER/luct_logs/debulx_docker_jenkins_run.log 2>&1
    #
    # Docker Run Jenkins
    podman run -d --name LUCT_Jenkins -p 9104:8080 -p 50000:50000 --restart=always -v jenkins_data:/var/jenkins_home jenkins/jenkins:latest-jdk21 >> /home/$SUDO_USER/luct_logs/luct_start_jenkins.log 2>&1
    #
    # Kopie maken van Initieel Wachtwoord bestand vanaf Docker Volume naar lokale omgeving 
    # 25 juli 25 check gedaan
    # sudo docker exec -it 82d3e48892f5 ls /var/jenkins_home/secrets
...
function debulx_nested_podman_complete () {
    ...
    echo 'DEBIAN/UBUNTU - Step 3 of 9 Starting Local Registry Container'
    podman run -d -p 9105:5000 --restart always --name registry registry > /home/$SUDO_USER/luct_logs/luct_start_registry.log 2>&1
    echo 'DEBIAN/UBUNTU - Step 4 of 9 Starting Portainer Container Management Container'
