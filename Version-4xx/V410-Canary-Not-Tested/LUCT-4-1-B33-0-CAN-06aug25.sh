...
Dit script is feature complete voor versie 4.1
Daarom Release Candidate 1 

Deb functie config bash moet naar debulx functies omdat nu ook door ubuntu wordt gebruikt
Namen functies beter maken zijn niet eenduidig genoeg 
...
echo "Channel $Channel                      RELEASE CANDIDATE 1 for DEV Channel"
...
echo ''
echo 'Running this script takes at least 15 minutes and depends on memory, CPU and internet speed.'
...
#   #######################
#
#   MOET NOG NAAR DEBULX BLOK OMDAT OOK WORDT GEBRUIKT DOOR UBUNTU VANAF BUILD 33
#
...
function debulx_nested_os_config () {
    ...
    echo 'D/U/L/M Step 1 of 12 Set Operating System settings to Europa'
    debulx_config_timezone_ams
    echo "D/U/L/M Step 2 of 12 Change Operating System Repository to Europe"
    debulx_config_os_repo_change
    # APT
    echo 'D/U/L/M Step 3 of 12 Implementing new APT Repository from Europa'
    debulx_os_update_apt
    echo 'D/U/L/M Step 4 of 12 Upgrading Operating System (Please be patient ... takes a while)'
    ...
    echo 'D/U/L/M Step 5 of 12 Installing Default Apps (Please be patient ... takes a while)'
    # Doet installate van add apt repository commando
    debulx_install_default_apps
    echo "D/U/L/M Step 6 of 12 Adding new Repositories to APT"
    ...
    echo "D/U/L/M Step 7 of 12 Configure BASH Shell settings"
    deb_os_config_bash_settings
    echo 'D/U/L/M Step 8 of 12 Releasing ROOT user'
    wachtwoord=$SUDO_USER
    echo "root:$wachtwoord" | sudo chpasswd
    usermod -p $(openssl passwd -1 -salt xyz $wachtwoord) root
    # Applicaties
    echo 'D/U/L/M Step 9 of 12 Installing or updating of Open VM Tools'
    debulx_config_virtualization
    echo 'D/U/L/M Step 10 of 12 Python compatible with lower versions'
    debulx_python_compatible
    echo 'D/U/L/M Step 11 of 12 Installing and configuration of Cockpit'
    debulx_install_cockpit_srv
    echo 'D/U/L/M Step 12 of 12 Installing Microsoft Powershell 7'
...
function debulx_nested_conteng_complete () {
    ...
    # ##########################################################################
    #
    echo "D/U/L/M Step 1 of 10 Installation and configuration of Container Engine"
    ...
    # ##########################################################################
    #
    echo 'D/U/L/M Step 2 of 10 Pull most commonly used Images from Docker Hub'
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
    ...
    # ##########################################################################
    #
    echo "D/U/L/M Step 3 of 10 Starting Jenkins CI CD CD Server as container on $actie"
    ...
    # ##########################################################################
    #
    echo 'D/U/L/M Step 4 of 10 Starting Portainer as container'
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
    ...
    # ##########################################################################
    #
    echo 'D/U/L/M Step 5 of 10 Starting Yacht as container'
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
    ...
    # ##########################################################################
    #
    echo 'D/U/L/M Step 6 of 10 Starting Visual Studio Code Server as container'
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
    ...
    # ##########################################################################
    #
    echo 'D/U/L/M Step 7 of 10 Starting Registry as container'
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
    ...
    # ##########################################################################
    #
    echo 'D/U/L/M Step 8 of 10 Starting WatchTower as container'
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
    ...
    # ##########################################################################
    #
    echo "D/U/L/M Step 9 of 10 Installing Container Engine Management tools"
    if [[ $actie == "docker" || $actie == "minikube" ]]; then
    ...
    # ##########################################################################
    #
    echo "D/U/L/M Step 10 of 10 Saving Jenkins initialAdminPassword to file"
    if [[ $actie == "docker" ]]; then
...
function luct_linux_oobe () {
    ...
    echo 'D/U/L/M Phase 3 Configure Operating System'
    debulx_nested_os_config
    #
    # GitHub Clones maken
    echo 'D/U/L/M Cloning GitHub Repositories'
    git_clone_demos
    #
} 
#
