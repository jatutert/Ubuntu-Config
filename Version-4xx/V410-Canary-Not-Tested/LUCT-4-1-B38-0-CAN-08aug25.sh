...
function debulx_install_conteng () {
    ...
    if [[ $actie = "docker" || $actie = "minikube" ]]; then
        # 
        echo 'Removing any installation of container daemon'
        ...
        echo "Adding $SUDO_USER to Docker security group"
...
function debulx_install_ansible_master_slave () {
    ...
    if [[ $rol == "master" ]]; then
        ...
        if [ -n "$ens_interface" ]; then
            ...
            # Aanpassen hosts bestand 
            if grep -q "$ens1_ip" /etc/hosts; then
                echo "$HOSTNAME already exists in /etc/hosts"
            else
                echo "$ens_ip $HOSTNAME" | sudo tee -a /etc/hosts > /dev/null
                # TO DO versie 4.2
                # Naam van de slaaf moet nog dynamisch worden gemaakt
                echo "$ens_plus1_ip D12-BKW-M-IAC-S01" | sudo tee -a /etc/hosts > /dev/null
                echo "Hostname $HOSTNAME added to /etc/hosts"
            fi
        fi
        # MASTER stap 3a
...
function debulx_nested_os_config () {
    ...
    echo '## Phase 3 - Step 1 of 12 Set Operating System settings to Europe'
    debulx_config_timezone_ams
    echo "## Phase 3 - Step 2 of 12 Change Operating System Repository to Europe"
    debulx_config_os_repo_change
    # APT
    echo '## Phase 3 - Step 3 of 12 Implementing new APT Repository from Europe'
    debulx_os_update_apt
    echo '## Phase 3 - Step 4 of 12 Upgrading Operating System (Please be patient ... takes a while)'
    ...
    echo '## Phase 3 - Step 5 of 12 Installing Default Apps (Please be patient ... takes a while)'
    # Doet installate van add apt repository commando
    debulx_install_default_apps
    echo "## Phase 3 - Step 6 of 12 Adding new Repositories to APT"
    ...
    echo "## Phase 3 - Step 7 of 12 Configure BASH Shell settings"
    debulx_config_bash_shell
    echo '## Phase 3 - Step 8 of 12 Releasing ROOT user'
    wachtwoord=$SUDO_USER
    echo "root:$wachtwoord" | sudo chpasswd
    usermod -p $(openssl passwd -1 -salt xyz $wachtwoord) root
    # Applicaties
    echo '## Phase 3 - Step 9 of 12 Installing or updating of Open VM Tools'
    debulx_config_virtualization
    echo '## Phase 3 - Step 10 of 12 Python compatible with lower versions'
    debulx_python_compatible
    echo '## Phase 3 - Step 11 of 12 Installing and configuration of Cockpit'
    debulx_install_cockpit_srv
    echo '## Phase 3 - Step 12 of 12 Installing Microsoft Powershell 7'
...
function luct_linux_oobe () {
    ...
    echo '## Phase 3 - Configure Operating System'
    debulx_nested_os_config
    #
    # GitHub Clones maken
    echo '## Phase 4 - Cloning GitHub Repositories'
    git_clone_demos
    #
} 
#
