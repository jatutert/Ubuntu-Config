...
function ulx_docker_vscodesrv_create () {
   ...
   # 
   CONTAINER_NAME="LUCT_VS_Code_Server"
   PROJECT_DIR="/home/$SUDO_USER/demos"
   DOCKER_IMAGE="codercom/code-server:latest"
   #
   # Start Visual Studio Code Server als Container
   #
   docker run -d --name "$CONTAINER_NAME" -p 9103:8080 -v "$PROJECT_DIR:/home/coder/project" "$DOCKER_IMAGE" --auth=none
   #
...
function ulx_docker_jenkins_create () {
    ...
    #
    # Docker Run Jenkins
    docker run -d --name LUCT_Jenkins -p 8201:8080 -p 50000:50000 --restart=always -v jenkins_data:/var/jenkins_home jenkins/jenkins:latest-jdk21
    #
...
function ulx_nested_oobe () {
    ...
    ulx_os_update_apt
    echo 'DEBIAN/UBUNTU - Step 2 of 7 Upgrading Operating System (Please be patient ... takes a while)'
    ulx_os_upgrade_os
    echo 'DEBIAN/UBUNTU - Step 3 of 7 Changing Domain Name Service (DNS) settings'
    ulx_os_config_dns
    # ulx_os_netplan_download
    echo 'DEBIAN/UBUNTU - Step 3 of 7 Installing Default Apps'
    ulx_os_default_apps
    echo 'DEBIAN/UBUNTU - Step 4 of 7 Installing or updating of Open VM Tools'
    ulx_install_vm_tools
    echo 'DEBIAN/UBUNTU - Step 5 of 7 Installing and configuration of Cockpit'
    ulx_install_cockpit
    echo 'DEBIAN/UBUNTU - Step 6 of 7 Installing Microsoft Powershell 7 (latest version)'
...
function ulx_nested_docker () {
    ...
    #
    echo 'DOCKER - Step 4 of 5 Starting Containers on Docker'
    #
    echo 'DOCKER - Step 4a Starting Registry'
    docker run -d -p 5000:5000 --restart always --name registry registry
    echo 'DOCKER - Step 4a Starting Portainer Container Management'
    ulx_docker_portainer_create
    echo 'DOCKER - Step 4b Starting Yacht Container Management'
    ulx_docker_yacht_create
    echo 'DOCKER - Step 4c Starting Jenkins'
    ulx_docker_jenkins_create
    echo 'DOCKER - Step 4d Starting Visual Studio Code Server'
    ulx_docker_vscodesrv_create
    #
    echo 'DOCKER - Step 5 of 5 Installing Docker Management tools'
