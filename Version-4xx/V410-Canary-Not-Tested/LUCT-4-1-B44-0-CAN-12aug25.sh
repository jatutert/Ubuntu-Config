...
Channel="Canary"
#
echo "LUCT $Major.$Minor Build $Build $Patch Channel $Channel Started by $USER" | sudo tee -a /var/log/luct.log
#
# #######################
# Blok 1D
...
# 12aug25  B42 Lus eth ens Ansible master nieuwe opzet EN syslogs aangezet
#
#
# #######################
# Blok 1F
...
echo ''
echo 'Running this script takes at least 17 minutes and depends on memory, CPU and internet speed.'
echo 'Running this script on a laptop running on battery power is NOT recommended.'
echo ''
#
#
# #######################
...
#
luct_start_time=$(date)
echo "Starttijd $luct_start_time" | sudo tee -a /var/log/luct.log
echo "Gebruiker $USER" | sudo tee -a /var/log/luct.log
#
logger "LUCT started"
#
#
...
#
phase_one_time=$(date)
echo "Phase 1 $phase_one_time" | sudo tee -a /var/log/luct.log
#
if [ $(id -u) -ne 0 ]; then
    clear
    luct_display_header
    ...
    logger "LUCT Version $Major.$Minor Build $Build Patch $Patch Terminated"
    exit 1
else
    echo '## Phase 1 - Checking executing rights'
fi
...
#
phase_two_time=$(date)
echo "Phase 2 $phase_two_time" | sudo tee -a /var/log/luct.log
#
...
if [ $# -eq 0 ]; then
    ...
    echo 'Terminate script execution ...'
    luct_end_time=$(date)
    echo "Einde $luct_end_time" | sudo tee -a /var/log/luct.log
    exit 1
fi
#
#
...
#
echo "Actie $actie" | sudo tee -a /var/log/luct.log
echo "modus $modus" | sudo tee -a /var/log/luct.log
#
# #######################
# Blok 2D
# Settings
...
#
#
phase_three_time=$(date)
echo "Phase 3 $phase_three_time" | sudo tee -a /var/log/luct.log
#
...
#
#
phase_four_time=$(date)
echo "Phase 4 $phase_four_time" | sudo tee -a /var/log/luct.log
#
...
#
#
phase_five_time=$(date)
echo "Phase 5 $phase_five_time" | sudo tee -a /var/log/luct.log
#
...
function debulx_os_upgrade_packages () {
    ...
    echo "Start OS Upgrade $luct_upgrade_os_start_time" | sudo tee -a /var/log/luct.log
    #
    if [[ $modus = "test" ]]; then
        apt full-upgrade -y
    ...
    echo "Einde OS Upgrade $luct_upgrade_os_end_time" | sudo tee -a /var/log/luct.log
    #
}
...
function debulx_install_default_apps () {
    ...
    echo "Start Installatie Default Apps $luct_install_default_apps_start_time" | sudo tee -a /var/log/luct.log
    #
    if [[ $modus == "test" ]]; then
        APT_INSTALL_ARRAY=(
            "7zip"
    ...
    echo "Einde Installatie Default Apps $luct_install_default_apps_end_time" | sudo tee -a /var/log/luct.log
    #
}
#
#
    ...
    if [[ $rol == "master" ]]; then
        ...
        if [ -n "$network_interface" ]; then
            ...
            if grep -q "$network_ip" /etc/hosts; then
                ...
                echo "$network_plus1_ip D12-BKW-M-IAC-S01" | sudo tee -a /etc/hosts > /dev/null
    ...
    if [[ $rol == "slave" ]]; then
        ...
        if [ -n "$ens_interface" ]; then
            ...
            if grep -q "$eth1_ip" /etc/hosts; then
                ...
                echo "$eth_ip $HOSTNAME" | sudo tee -a /etc/hosts > /dev/null
                echo "$eth_min1_ip D12-BKW-M-IAC-S01" | sudo tee -a /etc/hosts > /dev/null
                echo "Hostname $HOSTNAME added to /etc/hosts"
            fi
        fi
        # SLAVE stap 2
...
function debulx_conteng_portainer_run () {
    #
    echo "Portainer" | sudo tee -a /var/log/luct.log
    #
    if [[ $modus == "test" ]]; then
        echo 'Portainer'
    fi
...
function debulx_conteng_yacht_run () {
    #
    echo "Yacht" | sudo tee -a /var/log/luct.log
    #
    if [[ $modus == "test" ]]; then
       echo 'Yacht'
    fi
...
function debulx_conteng_vscsrv_run () {
    #
    echo "Visual Studio Code Server" | sudo tee -a /var/log/luct.log
    #
    if [[ $modus == "test" ]]; then
...
function debulx_conteng_jenkins_run () {
    #
    echo "Jenkins" | sudo tee -a /var/log/luct.log
    #
    if [[ $modus == "test" ]]; then
       echo 'Jenkins'
    fi
...
function debulx_conteng_registry_run () {
    #
    echo "Registry" | sudo tee -a /var/log/luct.log
    #
    if [[ $modus == "test" ]]; then
        echo 'Registry'
    fi
...
function debulx_conteng_watchtower_run () {
    #
    echo "WachTower" | sudo tee -a /var/log/luct.log
    #
    if [[ $modus == "test" ]]; then
        echo 'WatchTower'
    fi
...
function debulx_config_add_repositories () {
    ...
    # ############################
    #
    echo "Microsoft" | sudo tee -a /var/log/luct.log
    #
    ...
    # ############################
    #
    echo "Kubernetes" | sudo tee -a /var/log/luct.log
    #
    ...
    # ############################
    #
    echo "Ansible" | sudo tee -a /var/log/luct.log
    #
    if [[ $distro == "ubuntu" ]]; then
...
function debulx_config_bash_shell () {
    #
    echo "Bash Shell Config" | sudo tee -a /var/log/luct.log
    #
...
function debulx_nested_os_config () {
    ...
    echo "Phase 8 $phase_eight_time $USER $SUDO_USER" | sudo tee -a /var/log/luct.log
    #
...
function debulx_nested_conteng_complete () {
    #
    phase_ten_time=$(date)
    echo "Phase 10 $phase_ten_time $USER $SUDO_USER" | sudo tee -a /var/log/luct.log
    #
    # ##########################################################################
    #
...
function verander_machinenaam () {
    #
    phase_six_time=$(date)
    echo "Phase 6 $phase_six_time" | sudo tee -a /var/log/luct.log
    #
...
function maak_directories () {
    ...
    echo "Phase 7 $phase_seven_time" | sudo tee -a /var/log/luct.log
    #
    #
    # Temporary
    #
...
function git_clone_demos () {
    ...
    echo "Phase 9 $phase_nine_time" | sudo tee -a /var/log/luct.log
    #
    # Demos
