...
function deb_os_change_repo_nl () {
    ...
    if [ $VERSION_ID == "12" ] ; then
        ...
        add-apt-repository http://mirror.nl.datapacket.com/debian/ -s -y
        #
    fi
}
#
...
#
#   #######################
#   3D16 Debian OS FUNCTIES
#        Functie Change DNS OS
#        Onderdeel van Debian Nested OOBE Functie
#   #######################
#
#
function deb_os_config_dns () {
    #
    #   #########################################################
...
#
#
# 3D2 CATEGORIE UBUNTU OS Install Software Functies
#
#
#
#
...
#
#
function deb_install_pwrshell () {
    # Vullen variable Version ID
    source /etc/os-release
    #
    curl -s -SL "https://packages.microsoft.com/config/debian/$VERSION_ID/packages-microsoft-prod.deb" -o "/tmp/packages-microsoft-prod.deb"
...
#
#
# ################################### EINDE Debian INSTALLATIE ###############################
#
#
#
#
# 3D3 CATEGORIE Debian OS Nested Functions
#
#
# 3D31 Debian OS Nested Functions Basis Configuratie
#
#
#
function deb_nested_oobe () {
    ...
    echo 'DEBIAN - Step 2 of 12 Configure BASH shell'
    deb_os_bash_config
    echo 'DEBIAN - Step 3 of 12 Change Debian Repository Settings'
    deb_os_change_repo_nl
    echo 'DEBIAN/UBUNTU - Step 4 of 12 Updating APT Package Manager'
    debulx_os_update_apt
    echo 'DEBIAN/UBUNTU- Step 5 of 12 Upgrading Operating System (Please be patient ... takes a while)'
    debulx_os_upgrade_packages
    echo 'DEBIAN - Step 6 of 12 Changing Domain Name Service (DNS) settings'
    deb_os_config_dns
    echo 'DEBIAN - Step 7 of 12 Configure Network Interface Card 2'
    # ulx_os_netplan_download
    echo 'DEBIAN - Step 8 of 12 Installing Default Apps'
    deb_os_default_apps
    echo 'DEBIAN/UBUNTU - Step 9 of 12 Installing or updating of Open VM Tools'
    debulx_virtualization_configure
    echo 'DEBIAN/UBUNTU - Step 10 of 12 Installing and configuration of Cockpit'
    debulx_install_cockpit_srv
    echo 'DEBIAN - Step 11 of 12 Installing Microsoft Powershell 7 (latest version)'
    deb_install_pwrshell
    echo 'DEBIAN/UBUNTU - Step 12 of 12 Installing Python 3'
...
#
#
# 3DU1 CATEGORIE UBUNTU OS FUNCTIES
#
#
#
#
...
#
#   #######################
#   3DU12 Debian Ubuntu OS FUNCTIES
...
#        Functie Installatie OS Default Apps
#        Onderdeel van Ubuntu Nested OOBE Functie
#   #######################
#
#
...
function debulx_install_default_apps () {
    ...
    # Onderstaande zorgt ervoor dat add-apt-repository beschikbaar komt 
...
#
#
function debulx_virtualization_configure () {
    #
...
#        Onderdeel van Docker Nested Functie
#   #######################
#
#
function debulx_install_docker_ce () {
    ...
    if ! [ -x "$(command -v podman)" ]; then
        ...
        if ! [ -x "$(command -v docker)" ]; then
            ...
            #
            /snap/bin/curl -sSL https://get.docker.com/ | sh > /home/$SUDO_USER/luct_logs/debulx_install_docker_ce.log 2>&1
            #
...
#        Onderdeel van Podman Nested Functie
#   #######################
#
#
...
#
#
# 3DU5 CATEGORIE Debian UBUNTU APPS Nested Functions
#
#
#
#
#   #################################
#   3DU50 Debian Ubuntu Nested 
#         Docker
#   #################################
#
#
...
#
#   #######################
#   3U25 UBUNTU OS Install Software Functies
...
function ulx_nested_oobe () {
    ...
    # ulx_os_netplan_download
    echo 'UBUNTU - Step 7 of 11 Installing Default Apps'
    ...
    debulx_install_cockpit_srv
    echo 'UBUNTU - Step 10 of 11 Installing Microsoft Powershell 7 (latest version)'
...
if [ $distro == "debian" ]; then
    ...
    if [ $actie == "upgrade" ]; then
        ...
        if [[ "$SUDO_USER" == "debian" || "$SUDO_USER" == "osboxes" ]]; then
            NEW_HOSTNAME="D12-LTS-M-DKR-001"
