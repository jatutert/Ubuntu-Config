...
function debulx_install_conteng () {
    ...
    if [[ $actie = "docker" || $actie = "minikube" ]]; then
        ...
        /snap/bin/curl -fsSL https://download.docker.com/linux/debian/gpg -o /etc/apt/keyrings/docker.asc
...
function debulx_install_pwrshell () {
    ...
    if [ $? -ne 0 ]; then
        /snap/bin/curl -s -SL "https://packages.microsoft.com/config/$distro/$versie/packages-microsoft-prod.deb" -o "/tmp/packages-microsoft-prod.deb"
...
function debulx_config_os_repo_add_new () {
    ...
    if [[ $distro == "ubuntu" ]];  then
        # US 100GBs
        add-apt-repository http://mirror.enzu.com/ubuntu/ -s -y > /dev/null 2>&1  
    fi
    # US 50GBs
