...
function ulx_nested_docker () {
    echo 'DOCKER - Step 1 of 9 Installation and configuration Docker CE with Compose plugin'
    ulx_install_docker
    echo 'DOCKER - Step 2 of 9 Pull most commonly used Docker images from Docker Hub'
    ulx_docker_images_pull
    echo 'DOCKER - Step 3 of 9 Starting Registry'
    docker run -d -p 9105:5000 --restart always --name registry registry > /home/$SUDO_USER/luct_logs/luct_start_registry.log 2>&1
    echo 'DOCKER - Step 4 of 9 Starting Portainer Container Management'
    ulx_docker_portainer_create
    echo 'DOCKER - Step 5 of 9 Starting Yacht Container Management'
    ulx_docker_yacht_create
    echo 'DOCKER - Step 6 of 9 Starting Jenkins'
    ulx_docker_jenkins_create
    echo 'DOCKER - Step 7 of 9 Starting Visual Studio Code Server'
    ulx_docker_vscodesrv_create
    echo 'DOCKER - Step 8 of 9 Installing Docker Management tools'
    ulx_docker_mgmt_tools
    echo 'DOCKER - Step 9 of 9 Saving Jenkins initialAdminPassword to file'
    docker cp LUCT_Jenkins:/var/jenkins_home/secrets/initialAdminPassword /home/$SUDO_USER/Jenkins_Wachtwoord.txt
    echo '##################################################'
    echo ''
}
