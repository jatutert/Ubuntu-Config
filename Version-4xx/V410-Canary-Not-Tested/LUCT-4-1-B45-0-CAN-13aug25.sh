...
echo "Starttijd $luct_start_time" | sudo tee -a /var/log/luct.log > /dev/null 2>&1
echo "Gestart door gebruiker $USER" | sudo tee -a /var/log/luct.log > /dev/null 2>&1
#
...
function luct_finish_script () {
    #
    luct_end_time=$(date)
    echo "Einde $luct_end_time $USER $SUDO_USER" | sudo tee -a /var/log/luct.log > /dev/null 2>&1
    #
    clear
    luct_display_header
    # Test modus 
    if [[ $modus == "test" ]]; then
        ...
        logger 'LUCT finished on test mode'
        exit 1 
    else
        logger 'LUCT Rebooting after finish script'
        shutdown -r now
    fi
# Actie klaar
}
